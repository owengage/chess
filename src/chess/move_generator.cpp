...
namespace chess {
    ...
    namespace
    {
        std::vector<Loc> king_locs(Board const& b)
        {
            std::vector<Loc> kings;

            for (auto const& loc : Loc::all_squares())
            {
                auto sq = b[loc];
                if (std::holds_alternative<King>(sq))
                {
                    kings.push_back(loc);
                }
            }
    ...
    bool MoveGenerator::is_in_check(Board const& b) {
        auto kings = king_locs(b);

        // can't generate moves without going into a recursive loop.
        return false;
    }
    ...
    void MoveGenerator::remove_checked(MoveList &ml) {
        auto new_end = std::remove_if(begin(ml), end(ml), [this](Move m) {
            return is_in_check(m.result);
        });
        ml.erase(new_end, end(ml));
    }
